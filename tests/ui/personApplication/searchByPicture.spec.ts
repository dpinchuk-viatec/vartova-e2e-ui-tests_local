...
test("Search by Picture: checking different features", async ({ page }) => {
    test.info().annotations.push({
        type: "test_id",
        description: "https://app.clickup.com/t/869a6mup6"
    });
    ...
    await test.step("Select 'Search By Picture' card", async () => {
        await homePage.clickOnMenuManagementCard(CardLinks.SearchByPicture);
    });
    ...
    await test.step("Upload some image with faces", async () => {
        await searchByPicturePage.uploadImage(searchByPicturePage.selectPictureButton, "image_with_faces.jpg");
    });

    await test.step("Verify uploaded image with faces", async () => {
        await selectTargetPage.verifyModalWindowIsOpened(true);
        await selectTargetPage.verifyUploadedImage(true, 6);
    });
