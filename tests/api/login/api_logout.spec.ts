...
import { ENCRYPT, FIND_MENU, LOGIN, LOGOUT } from "../../../api/controllers/path";
import { ApiMethods } from "../../../constants/ApiMethods";
import { bodyGenerator, headerGenerator } from "../../../utils/Tools";
import { CodeStatuses, LoginStatuses, MsgStatuses } from "../../../constants/EntityStatuses";
...
test("Test User Logout and Associated API Requests", async ({ request }) => {
    test.info().annotations.push({
        type: "test_id",
        description: "https://app.clickup.com/t/869a91peg"
    });
    ...
    await test.step("Execute Logout request", async () => {
        textToSign = getTextToSign(ApiMethods.get, LOGOUT);
        xCaSignature = hmacSha256Node(textToSign, credentials.SECRET_KEY);
        loginHeader = headerGenerator(xCaSignature, false, { CTGT: tgc });

        // Step #6. Execute GET /artemis/api/common/v1/tgt/logout request.
        const logoutRequest = await LoginController.logout(
            request,
            loginHeader,
            isContentType,
        );

        expect(logoutRequest.status()).toBe(StatusCode.SuccessOK);

        code = (await logoutRequest.json()).code;
        msg = (await logoutRequest.json()).msg;

        expect(code).toBe(CodeStatuses.Zero);
        expect(msg).toBe(MsgStatuses.Success.toLowerCase());
    });
    ...
    await test.step("Execute request '/findMenu'", async () => {
        ...
        // Step #7. Wait for the GET /artemis/api/common/v1/menuService/findMenu request.
        ...
        code = (await findMenuRequest.json()).code;
        msg = (await findMenuRequest.json()).msg;

        expect(code).toBe(CodeStatuses["0x1e9601000"]);
        expect(msg).toBe(LoginStatuses.LoginUserExpired);
    });
});